  ...
  return (
    <div className={`right-panel-menu fixed right-[-20px] top-1/3 z-50 font-Outfit font-bold text-sm text-[#7F7F7F] transform transition-transform -translate-x-20 ${props.isVisible ? '-translate-x-9' : 'translate-x-full'
      ...
      <ul className='space-y-4 list-disc'>
        <li className={props.stage === 'Introduction' ? 'active' : ''} onClick={() => handlePanelClick("homeDiv")}>Introduction</li>
        <li className={props.stage === 'Vision' ? 'active' : ''} onClick={() => handlePanelClick("visionDiv")}>Vision</li>
        <li className={props.stage === 'Features' ? 'active' : ''} onClick={() => handlePanelClick("feature")}>Features</li>
        <li className={props.stage === 'Early' ? 'active' : ''} onClick={() => handlePanelClick("earlyAccessDiv")}>Get early access</li>
      </ul>
    </div>
  );
};
